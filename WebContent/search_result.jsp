...
<body>
	...
	<%
		...
		ArrayList<String[]> result = SearchQuery.get_arrayList_from_search_query(query, att_order, order, (current_page-1)*display_count, display_count+1);
	...
	<div class="section">
		<table width="700" align="center" style="border: 2px solid #000000;">
			<tr>
				...
				<td><a href="search_result.jsp?att_order=<%=att_order%>&order=<%=order%>&current_page=1&display_count=10">10</a></td>
				<td><a href="search_result.jsp?att_order=<%=att_order%>&order=<%=order%>&current_page=1&display_count=25">25</a></td>
				<td><a href="search_result.jsp?att_order=<%=att_order%>&order=<%=order%>&current_page=1&display_count=50">50</a></td>
				<td><a href="search_result.jsp?att_order=<%=att_order%>&order=<%=order%>&current_page=1&display_count=100">100</a></td>
				<td><b>Title: </b></td>
				<td><a href="search_result.jsp?att_order=title&order=ASC&current_page=1&display_count=<%=display_count%>">ASC</a></td>
				<td><a href="search_result.jsp?att_order=title&order=DESC&current_page=1&display_count=<%=display_count%>">DESC</a></td>
				<td><b>Year: </b></td>
				<td><a href="search_result.jsp?att_order=year&order=ASC&current_page=1&display_count=<%=display_count%>">ASC</a></td>
				<td><a href="search_result.jsp?att_order=year&order=DESC&current_page=1&display_count=<%=display_count%>">DESC</a></td>
			</tr>
		</table>
	</div>
	...
	<div class="section">
		<a href="search_result.jsp?att_order=<%=att_order%>&order=<%=order%>&current_page=<%=back%>&display_count=<%=display_count%>"><input type="button" value="Back" <%=back_cond%>/></a>
		<a href="search_result.jsp?att_order=<%=att_order%>&order=<%=order%>&current_page=<%=next%>&display_count=<%=display_count%>"><input type="button" value="Next" <%=next_cond%>/></a>
	</div>
